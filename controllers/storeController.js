...
exports.createStore = async (req, res) => { //پوستی که به ادد میاد این فانکشن هندل میکنه
  ...
  //الان نمیخوایم ریدایرکتش کنیم توی صفحه اصلی میخوایم بفرستیمش توی صفحه خودش 
  // این کدی که بالا نوشتیم میاد و اول یه استور میسازه بعدش صبر میکنه تا ذخیره بشه و طبق کدی که نوشتیم 
  //قبل از ذخیره شدن اسلاگ میسازه
  //فهمیدم چرا اسلاگ جواب نمیده چون که سیو توی استور نیست فقط صبر میکنه تا ذخیره بشه و اسلاگ رو میسازه
  //کدی که قرمر رنگ است رو هم بخون
  ...
  //یخ چیز دیگه هم هست و تونم اینه که ما برای رسیدگی کردن به ارور میتونیم دو تاراه استفاده کنیم
  //یکی اینه که بیایم کل کد رو توی ترای کچ بنویسیم 
  //که ازش متنفرم :|
  // try {
  //   await Store.save();
  //   console.log("worked");
  ...
  //راه دوم اینه که بیام و یک فانکشن جدا برای رسیپگی به اررور بنویسم که راه بهتری است
  //والان اینو اجرا میکنیم





  // کد بالا فانکشن دزدی از توی ارور هندلینگ هستش داخلش فانکشن میره خروجیشم فانکشنه
  // exports.catchErrors = (fn) => {
    // return function(req, res, next) {
      // return fn(req, res, next).catch(next);
  // توضیح این فانکشنه اینه که ورودیش یه فانکشنی است که قراره بیاد یه روتی رو بسازه و چیزی بهش بفرسته
  //میاد و چک میکنه ایا اروری داره یا نداره 
  //یه میدلور داخلی میسازه
  //و اگه اروری بود کچ میکنه و اگر نبود ریکوست و ریسپانس رو تحویل فانکشنی میده که قراره کار راه یابی را انجام بده
  //پس بطور کلی فانکشن راه یاب رو میگیره
  //میاد یه فانکنی میسازه ک سه قسمت داره ریکوست و ریسپانس و بعدی 
  //بعدش میاد ریکوست و ریسپانس رو میفرسته برای اون فانکشنه و اگه اروری دید میاد و رسیدگی میکنه به این صورت که میندازه برای میدلور بعدی توی 
  // فایل اپ دات جی اس
  //بعدشم مشخص میشه که چه اروری بوده 
...
exports.getStores =  async (req, res) => {
  //خب الان میخوایم استور ها رو نشون بدیم
  //برای نشون دادن استور ها باید بدونیم چی میحوام نشون طرف بدیم
  //ما میخوایم صفحه این جوری باشه که بیاد و تعدادی از شاپ ها رو نشون بده بصورت تصادفی
  //پس برای همین نیاز داریم که داده ها رو فچ کنیم
  const stores = await Store.find();
  //کاری که فایند میکنه اینه که میره تموم استور ها رو پیدا میکنه
  res.render('stores', { title: 'stores' });
}

exports.editStore = async (req, res) => {
  res.json(req.params);
  //دلیل این که اینو این زیر نوشتم اینه که توی فیلم توی ایدی برای راه ها 
  //اومد و از دو نقطه ایدی استفاده کرد
  //من انتظار داشتم که برای هر تکه اش یه چیزی نشونم بده ولی فقط نوشت
  //id: "54564645653"
  //یعنی فقط پارامز همین ایدی دار ها است که توی راه مینویسیمشون
}